...
/*
            				...
            				resultado.rows.forEach(function(row, indexRow, listaRow) {
            					_contador++;
            					...
            					if (_primeiraVez) {
            						...
            						_dataGraficoLeads.datasets[1].data[_contador] += new Number(row[_numeroAcessos]);
            					}

            					_acessos += new Number(row[_numeroAcessos]);
            					_acessosPorPagina += new Number(row[_numeroAcessos]);

								_visitantes += new Number(row[_numeroVisitante]);
								_visitantesPorPagina += new Number(row[_numeroVisitante]);


								if (_diaSemanaMes == "d")
            						_labels.push(row[0] + '/' + row[1] + '/' + row[2]);
            					else if (_diaSemanaMes == "s")
            						_labels.push('Semana ' + row[0]);
            					else if (_diaSemanaMes == "m")
            						_labels.push(row[0] + '/' + row[1]);


            					_datasets.push(new Number(row[_numeroAcessos]));
            					_datasetPagina.push(0);
            					

								if (indexRow == listaRow.length-1) {
									_primeiraVez = false;

									_listaTodosDadosAnalytics.push({
            							IDPagina: item.pageId,
            							Labels: _labels,
            							Dataset: _datasets,
            							Acessos: _acessosPorPagina,
            							Visitantes: _visitantesPorPagina,
            							DataSetUnbounce: _datasetPagina
            						});
									...
									if (_i == lista.length-1) {
										_i = -1;

										$scope.listaPaginasUnbounce.forEach(function(pagina, indexPaginas, listaPaginas) {

											if (pagina.IDPagina == '') {
												_i++;

												pagina.Acessos = _acessos;
												pagina.Visitantes = _visitantes;
												pagina.Conversoes = _conversoes;

												if (_visitantes > 0)
													_taxaConversao = ((_conversoes / _visitantes) * 100);
												else 
													_taxaConversao = 0;

												pagina.TaxaConversao = _taxaConversao;

												$scope.paginasUnbounceSelecionada = pagina;
											} else {
												if ($scope.paginasUnbounceSelecionada == '') {
													$scope.paginasUnbounceSelecionada = pagina;
												}
												...
												ConsultarLeads.query({
													...
													if (resultadoLeads.length > 0) {
														...
														resultadoLeads.forEach(function(lead, index, lista) {
															...
															if (_jaExisteOEmail.length <= 0) {
																...
																_dataGraficoLeads.labels.forEach(function(labelGrafico) {
																	...
																	if (labelGrafico == _dataMask) {
																		...
																		_listaTodosDadosAnalytics.forEach(function(analytics) {
																			if (analytics.IDPagina == lead.pagina) {
																				analytics.DataSetUnbounce[_auxiliar]++;
																				return;
																			}
																		});
																	}
															...
															if (index == lista.length-1) {
																_conversoes += _contador;

																pagina.Conversoes = _contador;
																pagina.TaxaConversao = ((_contador / pagina.Visitantes) * 100);
															}
														});							
													}			
													...
													if (_i == listaPaginas.length-1) {
														if (_visitantes > 0)
															_taxaConversao = ((_conversoes / _visitantes) * 100);
														else 
															_taxaConversao = 0;

														$scope.acessos = _acessos;
														$scope.visitantes = _visitantes;
														$scope.conversoes = _conversoes;
														$scope.taxaDeConversao = _taxaConversao.toFixed(1);
